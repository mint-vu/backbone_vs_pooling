...
class PointTransformerV2(nn.Module):
    ...
    def forward(self, coord):
        ...
        feat = coord
        batch = torch.tensor([[i] * p for i in range(b)], device=coord.device).flatten().long()
        offset = batch2offset(batch)
        ...
        skips = [[points]]
        # print(f"Stage 0: {points[0].shape}")
        for i in range(self.num_stages):
            ...
            # print(f"Encoder Stage {i}: {points[0].shape} {cluster.shape}")
        ...
        for i in reversed(range(self.num_stages)):
            ...
            # print(f"Decoder Stage {i}: {points[0].shape} {cluster.shape}")
        coord, feat, offset = points
        return coord.reshape(b, p, d)
