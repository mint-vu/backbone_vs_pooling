...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self,
                 in_channels: int = 3,
                 width: int = 32,
                 blocks: List[int] = [1, 1, 1, 1, 1, 1],
                 strides: List[int] = [1, 2, 2, 2, 2, 1],
                 block: Union[str, Type[InvResMLP]] = 'InvResMLP',
                 nsample: Union[int, List[int]] = 32,
                 radius: Union[float, List[float]] = 0.15,
    ...
    def forward(self, p0, f0=None):
        ...
        for i in range(0, len(self.encoder)):
            ...
            print(f"Block {i}: {p0.shape} {f0.shape}")
        return f0.repeat(1,1,1024).permute(0,2,1)